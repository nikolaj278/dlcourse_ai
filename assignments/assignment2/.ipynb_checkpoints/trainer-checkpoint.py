...
class Trainer:
    ...
    def fit(self):
        ...
        loss_history = np.zeros(self.num_epochs)
        train_acc_history = np.zeros(self.num_epochs)
        val_acc_history = np.zeros(self.num_epochs)
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch_indices in batches_indices:
                ...
                loss = self.model.compute_loss_and_gradients(batch, batch_y)

                for param_name, param in self.model.params().items():
                    optimizer = self.optimizers[param_name]
                    param.value = optimizer.update(param.value, param.grad, self.learning_rate)
                #--------
                # print("[debug] grad magnitude:", np.max([np.linalg.norm(g.grad) for g in self.model.params().values()]))
                #--------
            ...
            # if epoch > 2 and (np.std(val_acc_history[epoch-3: epoch]) < 1e-3 or np.sum(np.diff(val_acc_history[epoch-3: epoch]) < 0) > 0):
            #     self.learning_rate *= self.learning_rate_decay
            ...
            print("Loss: {}, Train accuracy: {}, val accuracy: {}, val_acc_diff {}, learning rate {}" \
                  .format(round(batch_losses[-1], 4), round(train_accuracy, 4), round(val_accuracy, 4), val_accuracy - val_acc_history[epoch-1], self.learning_rate))

            loss_history[epoch] = ave_loss
            train_acc_history[epoch] = train_accuracy
            val_acc_history[epoch] = val_accuracy
